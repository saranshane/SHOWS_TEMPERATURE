...
void main(void)
{
    ...
    while(1)
    {
      ...
      // float ntc_value = 0;
       ...
       // ntc_value = (ADC10MEM) * (3.30) / (1023.00);    // mapping 10-bit conversion result of ADC to corresponding voltage

        //int int_part = ntc_value;                                   // Integer part of calculated ADC value
        //int decimal_part = (ntc_value - (float)int_part) * 10.0 ;  // Decimal part of calculated ADC value



        //int thermistor_adc_val;
        double ntc_value, thermistor_resistance, therm_res_ln, temperature;
          //thermistor_adc_val = analogRead(thermistor_output);
         ntc_value = (( (ADC10MEM) * 3.3) / 1023.0 );
         thermistor_resistance = ( ( 3.3 * ( 1.0 / ntc_value ) ) - 1 ); /* Resistance in kilo ohms */
         ...
         temperature = ( 1 / ( 0.001129148 + ( 0.000234125 * therm_res_ln ) + ( 0.0000000876741 * therm_res_ln * therm_res_ln * therm_res_ln ) ) ); /* Temperature in Kelvin */
          ...
          int int_part = temperature - 43;
          int decimal_part = (temperature - (float)int_part) * 10.0 ;
